...
* Commit messages and pull request descriptions share one format: a Conventional Commits title; a body of one or two sentences stating what changed and, where a reader needs it, its effect; a list of the specific changes where there are several; footers for issue references and `BREAKING CHANGE` where they apply. Nothing else: no rationale beyond that effect, no alternatives considered, no account of the session, of review, or of testing.
* No `Co-Authored-By`, `Claude-Session`, or other AI metadata trailers in commits or PR descriptions.
