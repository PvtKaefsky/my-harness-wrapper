...
* A reviewer that has approved the commit is not invoked again for it; its approval stands through later fixes. After a fix, re-stage and invoke only the reviewers that have not yet approved. Approval is a stated verdict, not an empty findings list.
* Wait for delegated work by blocking on it, with a tool that waits on its completion, or on its completion notice — never on a timer.
...
* An environment change is recorded in the environment repository: the checkout holding both `config/CLAUDE.md` and `env/setup.sh`, `config/CLAUDE.md` being the source of this file. If no such checkout is attached to the session, stop and ask where to record it. Never drop it, never write it into the project repository whose task raised it, and never mix it into that project's diff.
...
* Commit messages and pull request descriptions share one format: a Conventional Commits title; a body of one or two sentences stating what changed and, where a reader needs it, its effect; a list of the specific changes where there are several; footers for issue references and `BREAKING CHANGE` where they apply. Nothing else: no rationale beyond that effect, no alternatives considered, no account of the session or of review; one `Tested:` footer line stating how the change was verified is allowed. A pull request description is checked against this format before it is posted.
* No attribution line in a commit or a pull request description: no `Co-Authored-By` or `Claude-Session` trailer, no session link, and no "Generated with" or "Generated by" Claude Code footer.
