...
* A reviewer that has approved the commit is not invoked again for it; its approval stands through later fixes. After a fix, re-stage and invoke only the reviewers that have not yet approved. Approval is a stated verdict, not an empty findings list. A verdict of approval that carries a finding other than LOW counts as withheld for that finding until it is fixed and re-reviewed, or declined.
